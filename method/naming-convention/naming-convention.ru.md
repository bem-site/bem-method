...
> **Почему префиксы в именах блока не нужны?** Имя блока в БЭМ само по себе задаёт пространство имён для его элементов и модификаторов, поэтому префиксы вроде `b-`, `c-`, `g-` для отделения блоков от прочих классов не нужны: достаточно, чтобы само имя блока было семантичным (`menu`, `header`, `search-form`). Изначально префиксы использовались, но мы от них отказались — подробнее см. раздел [«История»](../history/history.ru.md).
