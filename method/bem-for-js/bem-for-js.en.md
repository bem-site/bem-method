...
* Use BEM principles and operate not in classes, but in blocks, elements, and modifiers.
  ...
  > **Important!** Examples written for the BEM methodology use pseudocode. . Real implementation examples are provided in the documentation for [ i-bem.js] ( https://ru.bem.info/technology/i-bem/v2/i-bem-js/) .
...
> For implementation examples, see the documentation for [ i-bem.js] ( https://ru.bem.info/technology/i-bem/current/i-bem-js-mods/) .
...
> For implementation examples, see the documentation for [ i-bem.js] ( https://ru.bem.info/technology/i-bem/v2/i-bem-js-mods/) .
...
We could declare the block behavior and get the ability to override each modifier separately on a new redefinition level. . The [ declaration] ( https://ru.bem.info/technology/i-bem/current/i-bem-js-decl/) can specify which block or element should respond to modifier changes.
...
```files
logo/ logo.css #Block's appearance logo.tmpl # Templates for generating the bock's HTML representation logo.js # Dynamic behavior of the block in the browser
```
...
The fastest way is to start applying the principles of the BEM methodology in your project and get your first results without using a specialized framework. . To see how to do this in practice, read the article [BEM isn't just about CSS] ( https://ru.bem.info/forum/163/) (in Russian) with jQuery examples.
