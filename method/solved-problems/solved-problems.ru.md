...
В этом случае CSS может быть оформлен с помощью вложенных селекторов. Для этого достаточно доопределить правила, уже написанные для `.item`:
...
Использование вложенных селекторов связывает независимые компоненты интерфейса: нет возможности исправить один компонент, не затронув стили другого.
...
### Использование вложенных селекторов в БЭМ

Методология БЭМ допускает использование вложенных селекторов.

Например, вложенные селекторы уместны, чтобы менять элементы в зависимости от состояния блока или заданной ему темы:
...
> **Важно!** Применение вложенных селекторов увеличивает связанность кода и делает его повторное использование невозможным.
