...
![Обычный сайт](https://img-fotki.yandex.ru/get/15561/158800653.0/0_111fbc_61755fac_orig)
...
![Блоки сайта](https://img-fotki.yandex.ru/get/15592/158800653.0/0_111fbb_2bb44507_orig)
 ...
 * увеличить `Шапку`, или
 * сделать еще одну страницу, где в `Шапке` нет `Поиска`.
...
![Блок поисковой формы](https://img-fotki.yandex.ru/get/15569/158800653.0/0_111fb9_8ae85b15_orig)
...
![Поле ввода и кнопка](https://img-fotki.yandex.ru/get/15527/158800653.0/0_111fb8_2537137c_orig)
...
Блоки (или элементы) могут следовать друг за другом в определенном порядке.
...
Имя блока должно быть уникальным и однозначно указывать, о каком блоке идет речь.
...
Такие ключевые слова нужно размещать в определенном порядке. Для этого подойдут форматы
...
В данном примере пространства имен `b` и `e` использованы, чтобы отделять узлы блоков от узлов элементов.
...
Конечный HTML для браузера получается из БЭМ-дерева страницы путем наложения шаблонов
...
На его основе, при помощи JavaScript-based шаблонизатора BEMHTML, в последствии создается HTML-разметка.
...
```
 * CSS-класс элемента — это `имя блока` и `имя элемента`, разделенные
 определенным символом (или символами).
...
подчеркивания для отделения имени блока от имени элемента (`block-name__element-name`), но вы можете выбрать другую схему.
...
С точки зрения шаблонного движка независимость блоков означает, что
  ...
  * блок может быть легко помещен в любое место в БЭМ-дереве;
 ...
 * Сделать в подвале еще одно меню, с *другим дизайном*.
...
**Пример**
Модификатор блока задает ему цвет фона

![Смена фона блока](https://img-fotki.yandex.ru/get/16155/158800653.0/0_111fb7_46771936_orig)

**Пример**
Модификатор элемента для текущего пункта меню изменяет его внешний вид

![Смена внешнего вида блока](https://img-fotki.yandex.ru/get/16102/158800653.0/0_111fb4_6b15e133_orig)
...
Мы используем одно подчеркивание для отделения имени модификатора от имени блока и еще одно подчеркивание для отделения значения модификатора от его имени.
...
(метка) в нем - `tag`. Это справедливо для любой технологии: CSS, JS, XSL и т.д.
...
![Кнопка](https://img-fotki.yandex.ru/get/16130/158800653.0/0_111faf_308d2206_orig)

При наведении курсора на этот блок, он меняет свой внешний вид.

![Кнопка при наведении](https://img-fotki.yandex.ru/get/15574/158800653.0/0_111fae_1e5a7498_orig)
...
Таким образом, блок должен знать о себе все: реализация блока подразумевает описание его
...
В список технологий входит все, что относится к блоку.

### Примеры из реальной жизни
Наиболее широко методологию БЭМ применяет в своих frontend-разработках компания [Яндекс](https://company.yandex.ru).

БЭМ-методология не требует применения определенного фреймворка. Также, не обязательно
