...
БЭМ - очень гибкая методология, что позволяет вам настраивать ее под свои процессы, под свои текущие технологии.

В ситуации, когда
  * проект живет долго и постоянно обрастает новыми сущностями, и
  * команда разработки состоит из нескольких специалистов, растет и изменяется (появляются новые люди),
...
В большом проекте, если весь CSS-код блоков содержится в одном файле, в нем становится сложно ориентироваться.

Логичнее размещать код каждого блока в отдельном файле, одноименном имени блока:
...
При этом можно использовать такую схему расположения на файловой системе:
  ...
  * Основной код блока размещен в файлах `имя-блока`
...
В этом случае становится рационально использовать [bem-tools](https://ru.bem.info/tools/bem/bem-tools/) для сборки проекта.
...
Чтобы было легко найти нужный код, при разработке удобно следовать фиксированным требованиям:
  ...
  * Весь код блока размещен в отдельной директории
...
Очевидно, что поддержка такой файловой структуры в ручном режиме неудобна. Для автоматизации процессов мы создали [bem-tools](https://ru.bem.info/tools/bem/bem-tools/) - пакет инструментов, который помогает создавать
...
Если группа блоков одного проекта, объединенная в папку, подключена к другому проекту напрямую
...
При разработке нового сайта на основе существующего может потребоваться
  * увеличить шрифт в `Шапке` на всем сайте, или
...
![Набор реализаций блоков](https://img-fotki.yandex.ru/get/16135/158800653.0/0_111fcb_d9a605b5_orig)

Реализация любого блока библиотеки может быть расширена (или переопределена) на уровне проекта.

![Переопределение на уровне проекта](https://img-fotki.yandex.ru/get/15506/158800653.0/0_111fca_4f068470_orig)
...
open-source библиотека блоков [bem-bl](https://ru.bem.info/libs/bem-bl/).
...
путем применения реализаций описанных блоков.
...
**С точки зрения CSS**:
  * Файлы всех блоков собираются в один файл страницы.
    ...
    утилиты [borschik](https://ru.bem.info/tools/optimizers/borschik/).
  * Браузер получает минимизированный код.
    ...
    [CSSO](https://ru.bem.info/tools/optimizers/csso/).
...
Для сборки файлов страниц используется набор инструментов [bem-tools](https://ru.bem.info/tools/bem/bem-tools/).
...
страницы генерируются ее CSS и JavaScript файлы, шаблоны и, в случае статической разработки, HTML.
...
файлов проекта и после успешного выполнения сборки отдает их клиенту.

[Подробнее о командах](https://ru.bem.info/tools/bem/bem-tools/commands/).
