...
Основная идея соглашения — сделать имена CSS-селекторов максимально информативными и понятными. Это поможет [упростить разработку и отладку](problems.ru.md) кода, а также решить многие проблемы веб-разработчиков.

>Описание решений с подробными примерами собраны в разделе [Проблемы и способы их решения с БЭМ](problems.ru.md).

Здесь для наглядности приведем короткий пример. Рассмотрим CSS-селектор `menuitemvisible`. Быстрый просмотр такой записи не дает возможности определить типы БЭМ-сущностей по имени. Добавим разделитель:
