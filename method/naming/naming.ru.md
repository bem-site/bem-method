...
Основная идея соглашения — сделать имена CSS-селекторов максимально информативными и понятными. При первом прочтении имени, разработчик должен понимать, о какой сущности идет речь.

Жесткие правила создания имен БЭМ-сущностей упрощают разработку и отладку кода. Только за счет соглашения по именованию CSS-селекторов БЭМ решает многие проблемы веб-разработчиков.

>Описание решений с подробными примерами собраны в разделе [Проблемы и способы их решения с БЭМ](problems.ru.md).

>Здесь приведем только короткий пример улучшения информативности имен сущностей. Рассмотрим CSS-селектор `menuitemvisible`. Быстрый просмотр такой записи не дает возможности определить типы БЭМ-сущностей по имени. Добавим разделитель:

>`menu-item-visible` или `menuItemVisible`

>В таком виде имя селектора содержит явное выделение логических частей. Можем предположить, что `menu` окажется блоком, `item` — элементом, а `visible` — модификатором. Однако в реальной жизни веб-разработчик часто сталкивается с более сложными и не столь однозначными случаями, решить которые помогает соглашение по именованию БЭМ.

БЭМ-методология предоставляет идею по созданию правил формирования имен  и свой способ реализации — «Классическую схему именования». Однако наряду с классическим стилем существует ряд [альтернативных схем](#name-scheme), основанных на принципах БЭМ.
...
В БЭМ-методологии модификатор не может использоваться в отрыве от своего владельца.
...
>*Неверно*
>В данном случае модификатор указан без самого блока, на который он влияет.
...
### Пример применения классической схемы

Реализация авторизационной формы в HTML и CSS по принципам БЭМ:
...
Существуют альтернативные решения принципов именования БЭМ.
...
* Для записи имен используется [CamelCase](https://ru.wikipedia.org/wiki/CamelCase).
...
Методология БЭМ предлагает общие принципы именования БЭМ-сущностей, сформулированные для решения существующих проблем веб-разработки. Наличие классической и альтернативных схем именования, основанных на этих принципах, предоставляет возможность выбора наиболее подходящего решения для вашего проекта.
...
БЭМ-методология не ограничивается использованием технологий HTML и CSS, рассмотренных в этом документе. Понятия блоков, элементов и модификаторов применяются при работе с JavaScript, шаблонами и файловой системой БЭМ-проекта. Инструмент [bem-naming](https://ru.bem.info/tools/bem/bem-naming/#%D0%91%D0%AD%D0%9C-%D0%BD%D0%BE%D1%82%D0%B0%D1%86%D0%B8%D1%8F) позволяет использовать одинаковые имена БЭМ-сущностей во всех перечисленных технологиях. По умолчанию bem-naming содержит настройки классического стиля, но позволяет добавлять альтернативные правила для использования новых схем именования.
