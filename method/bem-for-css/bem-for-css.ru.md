...
Позволяют указать конкретный HTML-элемент страницы, независимо от тега. Обращение к селектору класса происходит через атрибут `class`, который должен имет каждый HTML-элемент страницы.
...
Выше рассмотрены значения атрибута `class`, состоящие из одного слова. В HTML значением атрибута `class` может быть разделенный пробелами список слов. Это позволяет использовать разные [БЭМ-сущности](../key-concepts/key-concepts.ru.md#БЭМ-сущность) на одном DOM-узле.
...
<header class="header">
    <!--
    `header__button` — микс (элемент блока `header`);
    `button` — блок;
...
Нужно избегать стилей, в которых совмещены теги и классы в селекторе, потому что повышается специфичность CSS-правил. Это усложняет задачу их переопределения и приводит к войнам значимости, при которых таблицы стилей загружаются излишне сложными селекторами.
...
Нужно стараться, чтобы селекторы были как можно короче.
...
* одинаково стилизовать разные HTML-элементы.
...
Как видно, между кнопками есть небольшие внешние отличия. В принципе DRY речь идет как раз о таких сущностях, функционально однотипных, но разных по оформлению.
