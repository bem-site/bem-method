...
Чтобы начать реализовать идеи БЭМ в вашем проекте, необходимо:

* абстрагироваться от DOM-модели и научиться мыслить блоками;
* не использовать ID-селекторы и селекторы тегов;
* минимизировать количество вложенных селекторов;
* использовать соглашение по именованию CSS-классов, для того чтобы избежать коллизии имен и сделать имена селекторов максимально информативными и понятными;
...
### Как начать постепенно внедрять идеи БЭМ в существующий проект?

* Не стоит сразу переписывать весь код, создавайте новые компоненты по БЭМ, а старые оставьте как есть.
* Проектируйте блоки, руководствуясь принципами, описанными выше.
* Используйте префиксы в именах CSS-классов (например, `bem-`), чтобы выделить новые блоки.
* Изменяйте первоначальный код по мере необходимости.

После знакомства с CSS переходите к рассмотрению особенностей реализации JavaScript по БЭМ-методологии.
