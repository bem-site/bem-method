...
Узлы связаны между собой отношениями «родительский-дочерний». Как правило, в CSS по БЭМ принято работать с каждым блоком так, как будто у него нет родительских связей.
...
**Используйте префиксы в именах CSS-классов (например, `bem-`) для разрешения конфликтов имен.** Создавая компоненты по БЭМ, вы можете по неосторожности переопределить свойства существующего блока. Предотвратить конфликт имен позволяют префиксы:
...
**Минимизируйте количество вложенных селекторов.**
