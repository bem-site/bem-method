...
```
Изменения коснутся всех блоков `button` на проекте.
...
Пытайтесь минимизировать случайные повторения. Перепишем пример в соответствии с принципом DRY:
...
Ниже описаны некоторые советы, позволяющие начать реализовать идеи БЭМ в вашем проекте:

**Абстрагируйтесь от DOM-модели и создавайте новые блоки по БЭМ.**

Согласно DOM-модели, документ является иерархией, деревом. Каждый HTML-тег образует узел дерева с типом «элемент»
Узлы связаны между собой отношениями «родительский-дочерний». Как правило, в CSS по БЭМ каждый блок разрабатывается так, как будто на странице ничего кроме него нет.

Из этого можно вывести несколько общих правил:
 * проектируйте новые блоки без учета возможности наследования;
 * работайте в единых терминах блоков, элементов и модификаторов.

**Используйте префиксы в именах CSS-классов для разрешения конфликтов имен.**

Создавая компоненты по БЭМ, вы можете по неосторожности переопределить свойства существующего блока. Предотвратить конфликт имен позволяют префиксы (например, `bem-`):

CSS-реализация:

```css
.button {}        /* реализация кнопки до использования БЭМ */
.bem-button {}    /* реализация кнопки по БЭМ */
```

**Рефакторинг**

Значение слова рефакторинг довольно размыто: так называют исправление дефектов, реализацию новой функциональности, модификацию проекта — по сути любое изменение кода. Это неуместно. В нашем случае под рефакторингом мы будем понимать выделение независимых блоков на странице и переписывание кода по БЭМ.

Причины выполнения рефакторинга:
* код повторяется;
* код имеет высокую связность из-за вложенных селекторов.

Вносите изменения поблочно. Несколько простых советов:

* начните с простых блоков;
* сохраняйте первоначальный код;
* определите зависимости.

**Не используйте ID-селекторы и селекторы тегов.**

По сути, использовать селекторы тегов и идентификаторов не имеет никакого смысла. Они не дают ничего, что нельзя было бы получить с применением простого селектора класса.

**Не создавайте вложенные селекторы.**

Простое добавление дополнительных селекторов с целью «победить» свойства стиля может привести к так называемым войнам значимости. Избегайте очень длинные и запутанные имена стилей вроде:
...
Над каким бы проектом вы не работали, вам следует принять соглашение по именованию CSS-классов. Выбирайте имена так, чтобы они облегчили чтение кода.

**Выносите в модификаторы CSS-свойства блока, изменение которых кажется вероятным.**

В случае, если у блока есть CSS-свойства, которые будут меняться (например, точно известно, что кнопки будут разного цвета и размера), лучшим решением будет выносить их в модификаторы и применять по необходимости.

**Повторно используйте блоки и создавайте собственные библиотеки.**

Хороший способ быстро создать сложную реализацию состоит в том, чтобы не писать ее полностью, а использовать готовые библиотеки блоков. При разработке нового блока, всегда думайте о том, что из уже реализованного ранее можно взять за основу.
...
**Определите действия, которые могут быть выполнены над конкретным блоком и переходите на JS по БЭМ.**

В БЭМ JavaScript используется для «оживления» веб-страницы и рассматривается как одна из технологий реализации блока.
