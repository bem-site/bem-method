...
> **Важно!** При миксовании блоков их CSS-свойства не должны пересекаться. Общие свойства уносятся в блок-миксин (`text`), а специфичные для секции — остаются в своём блоке (`article`, `copyright`). Если одно и то же CSS-свойство определено в обоих блоках, результат будет зависеть от порядка CSS-правил — это особенность каскада в CSS, а не специфика БЭМ. Та же дисциплина применяется и в разделе [Внешняя геометрия и позиционирование](#внешняя-геометрия-и-позиционирование) выше: блок `button` отвечает только за внутренние стили, а элемент `header__button` — только за внешнее размещение, поэтому конфликта свойств не возникает.
