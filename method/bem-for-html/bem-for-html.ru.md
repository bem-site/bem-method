...
Микс с содержимым здесь не подойдет: содержимое универсальной кнопки заранее неизвестно — там может оказаться иконка, текст или произвольный другой блок. Заранее «приклеить» к каждому такому блоку элемент `button__icon` или подобный нельзя. Поэтому позиционированием внутри блока занимается отдельный элемент-обёртка `button__inner`, который не зависит от того, какой именно блок лежит внутри.
