...
В БЭМ-методологии многие блоки строятся на основании других блоков. Для явного указания зависимостей от иных БЭМ-сущностей и технологий используются файлы-технологии `deps.js`. В качестве примера рассмотрим форму поиска (`search-form`) построенную с помощью блоков `input` (поле ввода) и `button` (кнопка).
