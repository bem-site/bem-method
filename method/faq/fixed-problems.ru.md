...
Например, для создания навигационного меню вполне могут использоваться следующие имена классов:
...
Если на такую страницу нужно добавить новые компоненты, содержащие пункты, то добавление стилей для еще одного `item` повлияет на пункты из навигационного меню.

Когда страницу верстает один разработчик в короткие сроки, коллизии имен можно избежать. Но если нужно внести правки спустя какое-то время, или в проект, который писал другой разработчиком, то отследить зависимые имена компонентов становится сложнее. В больших проектах результатом правки одного класса может стать десяток «разъехавшихся» страниц.

Рассмотрим на нашем примере. Предположим, что в навигационном меню нужно изменить правила класса `.active`. По имени непонятно, какие компоненты его используют. Может оказаться, что на другой странице существует, например, <div class=«button active»>Нажми меня!</div>. Тогда изменение правил для `.active` вызовут нежелательные «сюрпризы» в реализации стилей такой кнопки.
...
Рассмотрим этот же пример, но уже с примененными правилами именования БЭМ: класс `nav` будет обозначать имя блока, `nav__item` и `nav__link` — имена элементов, а `nav__item_active` — имя модификатора элемента `item`.
...
>* Длинные имена классов писать долго. С могут помочь автозаполнение в редакторе и инструментарий, который автоматически добавляет префиксы (CSS-препроцессоры и шаблонизаторы).
