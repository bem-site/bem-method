...
* селекторами и компонетами интерфейса, к которфм они относится.
...
Следование [соглашению по именованию сущностей](../naming/naming.ru.md) БЭМ позволяет понять структуру блока без подробного изучения HTML. Даже при появлении на странице еще одного поля (кроме `form__field`), его правила никак не будут влиять на элементы поисковой формы. Новое поле будет реализовано, как элемент нового блока, со своим специфичным именем. Например, `attach__field`.

Правила именования в БЭМ позволяют сделать код вашего проекта однозначным и, как следствие, информативным. Это снижает порог входа для других разработчиков.
...
Подобный код может работать до тех пор, пока не возникнет необходимость изменить страницу. Например, переместить какие-то пункты меню, использовать написанный код в другом месте отдельно от родительского компонента или вложить навигационное меню в блок `snippets`.
...
Изменения в `nav__item` не влияют на `snippets__item`, так как пункты получают уникальные имена благодаря неймспейсам в виде имен блоков. Это позволяет формировать независимые CSS-правила для всех элементов блока.
...
Как следствие, разработчик вынужден поддерживать большее количество строк кода. А исправления необходимо вносить отдельно в каждую реализацию, что увеличивает временные затраты на отладку проекта.
