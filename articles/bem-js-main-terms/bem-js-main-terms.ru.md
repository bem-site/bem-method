...
О том, почему нужно использовать `onclick`, можно узнать из доклада Сергея Бережного [Разные
способы создания клиентских
компонентов](https://events.yandex.ru/events/yasubbotnik/msk-jul-2012/talks/302/),
прозвучавшего на
...
Благодаря возможности считать параметры из `onclick` и получать нативный
JavaScript-объект, не нужны `id` компонент или парсинг CSS-классов. Все блоки,
помеченные классом `i-bem` превратятся в компоненты, соответствующие их параметрам.
