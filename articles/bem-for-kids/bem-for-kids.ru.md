# БЭМ не только про CSS, или несколько простых правил, которые заставят его полюбить

Повторюсь, БЭМ — это методология, созданная не только для нейминга селекторов CSS. Но начну я именно с примеров про CSS. Почему? Да всё просто, — так понятнее и лучше воспринимается. Но, дочитайте до конца. 

## Типичная верстка обычного проекта

Давайте рассмотрим типичный веб-проект. Даже если вы верстаете его самостоятельно и вам не нужно ни под кого подставиваться и договоариваться про нейминг, рано или поздно вам придется вернуться к своему коду для какого-нибудь рефакторинга, понять его, простить, и написать заново :) Или через какое-то время вы обнаружите, что так или иначе какие-то части удобно использовать не только в текущем проекте, но и скопировать их на соседний проект. 

Вот чтобы не было такого "понять, простить и переписать заново" стоит принять несколько правил.

Правило 1. Не используй ID и селекторы на тег
