...
Повторюсь, БЭМ — это методология, созданная не только для нейминга селекторов CSS. Придумывая имена и разделители в два подчеркивания, мы не  Но начнем именно с примеров про CSS. Почему? Да всё просто, — так понятнее и лучше воспринимается. Но, дочитайте до конца. 

## Типичная верстка обычного проекта

Рассмотрим типичный веб-проект. Даже если вы верстаете его самостоятельно и вам не нужно ни под кого подставиваться и договоариваться про нейминг, рано или поздно вам придется вернуться к своему коду для какого-нибудь рефакторинга, понять его, простить, и написать заново :) Или через какое-то время вы обнаружите, что так или иначе какие-то части удобно использовать не только в текущем проекте, но и скопировать их на соседний проект. 

Вот чтобы не было такого "понять, простить и переписать заново" стоит воспользоваться несколькими правилами: 

* Не используй ID и селекторы на тег
* Не злоупотребляй вложенными селекторами
* Попрощайся с универсальными селекторами и общим сбросом стилей
* Блок не должен ничего знать про позиционирование на странице




БЭМ — методология, которую используют многие курпные компание по всему миру. Что нужно сделать? изучить, понять, применить. Если сказать короче, нужно потратить на нее время. А стоит ли? Давайте разберемся. 

Давайте оговорим, чего мы ждем от применения БЭМ в своем проекте, не важно большом или в таком, где один челоек и дизайнер, и менеджер, и программист. 
...
В любом проетке есть кнопки, меню, шапки, логотипы, ссылки и так далее. 

А еще хочется, чтоб код был стабильным и предсказуемым. Звучит как план? Тогда разберем по шагам:

Первое, с чего стоит начать это — в БЭМ для декларации CSS-правил используются только классы. К сожалению, как аксиому мы записать это не можем, поэтому докажем как теорему, что так лучше, удобнее и так можно добиться критериев, которые мы перечислили выше. 


## Не используем идентификаторы (ID селекторы)

ID определяет уникальное имя элемента. Если имя уникально, второй раз в интерфейсе его вставить никак не получится. ID мешает повторно использовать верстку. Отметаем.


### Возможные исключения

Сейчас пытливый ум многих разработчиков зашевелился. А как же то, что ID можно использовать для JavaScript? Для JS вам тоже не нужно использовтаь ID. Современным браузерам совершенно не важно используете вы ID или классы. Браузер одинаково быстро обработает и тот и тот селектор. 
...
Да, бесспорно. Но в ситуации, когда вы можете завернуть ваш `<label>` в контрол, ID тоже становится не нужен. Конечно, если завернуть в контрол нельзя, тогда нужно оставлять ID.

Еще один годный случай использования ID - если нужно поставить якорь посреди страницы. 
...
2. Вложенность мешает, если нужно повторно использовтаь верстку или перенести часть интерфейса на новое место.
...
Позиционирование блока - миксы лока
