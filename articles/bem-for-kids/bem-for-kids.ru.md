...
* [Почему не используем селеторы атрибутов?](#Не-используем-общий-сброс-стилей-reset)
...
Второй распространенный случай применения ID — это тег `<label>`. Но, если поместить `<label>` в контрол, ID становится не нужен.
...
Набор тегов ограничен, поэтому при переносе верстки из одного проекта в другой вероятность столкнуться с конфликтами стилей для одних и тех же тегов сильно возрастает.
...
Попробуем написать разметку с помощью тегов. Для вставки каринки существует тег `<img>`, а для ссылок — тег `<a>`. 
...
В проекте все картинки будут выражены тегом `<img>`, и все ссылки — тегом `<a>`. Чтобы отличать логотип от картинки и ссылку с логотипа от обычной ссылки в контенте страницы, понадобятся отдельные стили. Для ссылки в логотипе необходимо отменить подчеркивание и синий цвет:
...
Ссылка с логотипа на всех страницах ведет на главную страницу сайта. Но на главной странице ее в логотипе быть на должно. Поэтому разметка изменится: 
...
На главной странице ссылка в логотипе выражена тегом `<span>`, для которого отменять подчеркивание и синий цвет не нужно. Поэтому общие правила для ссылки в логотипе запишем так::
...
Такой код не вносит ясности в проект. Если в новой версии дизайнер уберет логотип из макета, разработчику будет сложно определить, какие стили удалять из проекта.
...
Селектор `header a` также не информативен: он может содержать CSS-правила для ссылок в меню шапки, ссылки с логотипа или, например, ссылки на профиль автора. 

#### Возможные исключения

Методология в редких случаях допускает объединение селекторов тега и класса, например, для реализации блоков с комментариями в блогах или форумах любой CMS-системы. Пользователь может написать любой текст, вставить картинки, добавить свою разметку. Но чтобы оставаться в рамках дизайна сайта, разработчик может заранее определить стили всех возможных элементов.

Например, можно написать правила для основных тегов, которые доступны пользователю, и применять их каскадом к вложенным блокам:
...
Общий сброс стилей — это по сути те же глобальные CSS-правила, созданные для всей страницы. Все, что оказывает влияние на все узлы верстки, нарушает независимость компонентов и затрудняет их повторное использование.

В БЭМ не используется `reset` и `normalize` даже для отдельно вязтого блока. 
Во время «ресета» или «нормалайза» существующие стили отменяются и подменяются другими, которые потом в процессе работы приходится изменять и дорабатывать. В итоге разработчик вынужден писать стили, кторые перекрывают то, что только что сбросили с помощью `reset` или `normalize`.

### Не используем универсальный селектор (*)?

Универсальный селектор сообщает, что какой-то стиль повлиял на все узлы в верстке. Это накладывает ограничения на повторное использование верстки в другом проекте:
