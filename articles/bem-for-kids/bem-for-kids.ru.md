...
Первое распространенное мнение и заблуждение: ID обязателен для работы с JavaScript. 
...
Второй распространенный случай вынужденного применения ID — это тег `<label>`. 
...
Расширенный набор семантических тегов также не может выразить все потребности верстки. Рассмотрим распространенный пример макета: на странице расположена шапка с логотипом, по клику на который открывается главная страница сайта. 
...
Однако в проекте все картинки будут выражены тегом `<img>`, а все ссылки — тегом `<a>`. Чтобы отличать логотип от картинки и ссылку с логотипа от обычной ссылки в контенте страницы, понадобятся дополнительные стили. 

Отменим подчеркивание и синий цвет для ссылки в логотипе:
...
Ссылка в логотипе на главной странице выражена тегом `<span>`, для которого отменять подчеркивание и синий цвет не нужно. Поэтому общие правила для ссылки в логотипе запишем так::
...
Если в новой версии дизайнер уберет логотип из макета, разработчику будет сложно определить по именам селекторов, какие стили удалять из проекта.
...
Селектор `header a` также неинформативен: он не показывает связь ссылки с логотипом. Такой селектор может принадлежать ссылке в меню шапки, ссылке с логотипа или, например, ссылке на профиль автора. 

#### Возможные исключения

Методология в редких случаях допускает объединение селекторов тега и класса, например, для стилизации комментариев в блогах или форумах любой CMS-системы. В комментарии пользователь может написать любой текст, вставить картинки, добавить свою разметку. чтобы оставаться в рамках дизайна сайта, разработчик может заранее определить стили всех возможных HTML-элементов.
...
Общий сброс стилей — это по сути те же глобальные CSS-правила, созданные для всей страницы. Значит оказывает влияние на все узлы верстки, нарушает независимость компонентов и затрудняет их повторное использование.
