...
Первое распространенное мнение и заблуждение, что ID обязательно нужен для привязки JavaScript. Однако современным браузерам не важно, с какими данными работать: с ID или классами. Браузер одинаково быстро обработает любой селектор.

Второй распространенный случай применения ID — тег `<label>`. 
...
Однако, если завернуть `<label>` в контрол, ID становится не нужен. Конечно, если сделать это не получается, тогда ID придется оставить.

И третий допустимый случай  — установка якоря на странице. 

### Не используем селекторы тега

Селекторы тега нестабильны и изменчивы. HTML-разметка страниц часто меняется: теги передвигаются по странице, вкладываются друг в друга, удаляются. Новый дизайн сайта может поменять вложенность разделов и изменить уровень заголовоков с `<h1>` на `<h3>`, или превратить абзац `<p>` в тег `<div>`. 

Любые перечисленные действия с тегами ломают существующую разметку и стили. 

Если стили проекта написаны на селекторы тега, то перенос верстки в другой проект становится сложной задачей. Набор тегов крайне ограничен. При переносе верстки из проекта в проект вероятность столкнуться с конфликтами стилей, написанных на одни и те же теги, возрастает в разы. Стили будут перекрываться, накладываться и ломаться.

Даже расширенный набор семантических тегов не поможет, чтобы выразить все потребности верстки. Рассмотрим распространенный пример макета, в котором шапка содержит логотип, по клику на который открывается главная страница сайта. 

Попробуем воспользоваться тегами, чтобы выразить всё это в разметке. Для вставки каринки существует тег `<img>`. Но этот тег не может показать, что картинка — это логотип. Для перехода на главную страницу воспользуемся тегом `<a>`. Но в проекте все ссылки будут выражены тегом `<a>`, и нам нужно уметь отличать ссылку слоготипа от ссылки в навигационном меню и от обычной ссылки в контенте страницы. Все эти ссылки будут выражены тегом `<a>`, но все они будут иметь совершенно разную семантику. 
...
Чтобы создать разметку, не получится полностью полагаться на назания компонентов, из которых строится интерфейс. Ссылка логотипа со всех страниц ведет на главную страницу сайта, а на самОй главной странице эта же ссылка уже не нужна, так как она не должна никуда вести. И на главно странице, скорее всего, разработчик опишет ее тегом `<span>` или `<div>`, а не ссылкой `<a>`. 
...
Разберем пример со ссылками дальше. Ссылки в шапке должны выглядеть не так, как в тексте. Поэтому нужны отдельные стили для ссылки в шапке, где отменяется подчеркивание и синий цвет. 
...
Но у нас ссылка с логотипа на главной странице выражена тегом `<span>`, а для `<span>` отменять подчеркивание и синий цвет не нудно. Поэтому часть правил можно записать так:
...
Поэтому модификтаор всегда должен находиться на одном DOM-узле с блоком или элементом, к которому он относится.
