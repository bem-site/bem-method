...
* [С чего же все-таки начинать?](#С-чего-начать-)
...
На первый взгляд, такой код имеет право на существование. Но представьте ситуацию, в которой дизайнер убирает логотип из макета. Такие имена селекторов не дадут вам никакой информации, какие стили удалять из проекта вместе с логотипом. Селектор `header a` не показывает связь ссылки и логотипа. Такой селектор может принадлежать ссылке в меню шапки или, например, ссылке на профиль автора, а `header span` может относиться к любой части шапки.
...
Во время сброса или нормализации существующие стили отменяются и подменяются другими, которые потом в процессе работы необходимо изменять и дорабатывать. В итоге разработчик вынужден писать стили, кторые перекрывают то, что только что сбросили.
...
Универсальный селектор также может сделать код вашего проекта непредсказуемым. Например, повлиять на стили компонентов из универсальной библиотеки.

Общие стили не выигрывают время: часто разработчик начинает с того, что сбрасывает все отступы для универсальных компонентов (`* { margin: 0; padding: 0; }`), а потом все равно задает их как в макете (`margin: 12px; padding: 30px;`).

### Не используем вложенные селекторы

Вложенные селекторы увеличивают связность кода и затрубняют его повторное использование.
Методология БЭМ не запрещает вложенные селекторы, но рекомендует не злоупотреблять ими.
...
Совмещение тега и класса (например, `button.button`) повышает специфичность CSS-правил и затрудняет их переопределение. Высокая специфичность CCS-правил приводит к войнам значимости, при которых таблицы стилей загружаются излишне сложными селекторами.
