...
Альтернативой `ENB` может служить [bem-tools](https://ru.bem.info/tools/bem/bem-tools/). Результаты сборки в обоих случаях одинаковы, т.к. «под капотом» вызывается `enb make`.
...
4. [Укажем зависимости блоков](#deps) в файле `deps.js`, чтобы шаблоны, JavaScript-реализация и CSS-правила применились при сборке к нужным нам блокам.
...
<script src="https://gist.github.com/tadatuta/7beb85c518237538bb7c.js"></script>
...
**Важно** В `project-stub` по умолчанию подключен CSS-препроцессор [Stylus](https://learnboost.github.io/stylus/), который компилируется в CSS. Таким образом, у вас есть возможность создавать CSS-правила как в `.css`, так и в `.styl` форматах.
...
* использование библиотеки bem-core v2.8.0;
* использование ENB v1.1.0;
