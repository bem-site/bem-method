...
* [Когда создавать блок, когда — элемент?](#В-каком-случае-создавать-блок-в-каком--элемент)
* [Можно ли создавать элементы элементов (block\__elem1\__elem2)?](#Почему-в-БЭМ-не-рекомендуется-создавать-элементы-элементов-block__elem1__elem2)
...
* [Зачем использовать i-bem.js, если есть jQuery?](#Зачем-использовать-i-bemjs-если-можно-писать-на-jquery)

## CSS

* [Можно ли использовать вложенные селекторы?](#Почему-нежелательно-использовать-вложенные-селекторы)
* [Можно ли использовать комбинированные селекторы для CSS-правил к модификатору?](#Почему-в-БЭМ-не-рекомендуется-использовать-комбинированные-селекторы-для-создания-css-правил-к-модификатору)
...
* [Почему не используются пользовательские теги (custom tag) для блоков?](#Почему-в-БЭМ-не-используют-пользовательские-теги-custom-tags-для-блоков)
* [Почему не стоит делать общий сброс стилей (reset)?](#Почему-нельзя-делать-общий-сброс-стилей-reset)
* [Можно ли писать block_mod вместо block block_mod, так как в имени модификатора уже есть информация о блоке?](#Почему-нельзя-писать-block_mod-вместо-block-block_mod-если-имя-модификатора-уже-содержит-всю-информацию-о-блоке)
* [Можно ли указывать название CSS-свойства в имени модификатора: .block\__element\_border-color\_grey?](#Почему-нельзя-указывать-название-css-свойства-в-имени-модификатора-block__element_border-color_grey)
