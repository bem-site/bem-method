...
БЭМ можно использовать только на уровне CSS. Для этого достаточно следовать [рекомендациям методологии](../method/bem-for-css/bem-for-css.ru.md).
...
Явные и уникальные имена облегчают идентификацию сущностей в коде и на файловой структуре.
...
Имя модификатора показывает, о каком состоянии идет речь.

Если важно только наличие или отсутствие модификатора у блока, а его значение несущественно, следует использовать булевый модификатор. Например, модификатор, описывающий состояние «отключен»: `disabled`.
