...
* на уровне JavaScript (с помощью [переключения модификатора](https://ru.bem.info/platform/tutorials/i-bem/modifiers/#Переключение-toggle-модификатора)). Имена CSS-селекторов должны быть максимально конкретными. Например, `block_position_left`, `block_position_right`. Изменение CSS-классов на DOM-узле происходит при помощи JavaScript.
